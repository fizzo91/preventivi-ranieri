...
var import_zoho_project_draft_default = defineTool6({
  ...
  description: "Legge Deal, quotation e note da Zoho, scarica anche gli allegati incorporati nelle note in uno storage privato e genera un dossier con URL temporanei per l'agente tecnico. Se quote_id \xE8 presente importa quella quotation esatta. Non modifica Zoho o WorkDrive.",
  inputSchema: {
    deal_id: z6.string().regex(/^\d{10,25}$/).describe("ID numerico del Deal Zoho"),
    quote_id: z6.string().regex(/^\d{10,25}$/).optional().describe("ID numerico della Quotation Zoho da importare")
  },
