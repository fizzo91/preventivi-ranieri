...
export function buildTechnicalDossier(deal: ZohoRecord, quote: ZohoRecord | null, notes: ZohoRecord[], attachments: ZohoRecord[]) {
  ...
  return {
    ...
    items,
    notes: notes.map((note) => ({ id: note.id, source: note._source ?? null, title: note.Note_Title ?? null, content: note.Note_Content ?? "" })),
    attachments: attachments.map((file) => ({
      id: file.id,
      name: file.name ?? file.File_Name,
      size: Number(file.size ?? file.Size) || null,
      sourceModule: file.sourceModule ?? null,
      sourceLabel: file.sourceLabel ?? null,
      createdTime: file.createdTime ?? file.Created_Time ?? null,
      downloaded: file.downloaded ?? false,
      mimeType: file.mimeType ?? null,
      sha256: file.sha256 ?? null,
      storagePath: file.storagePath ?? null,
      signedUrl: file.signedUrl ?? null,
      signedUrlExpiresIn: file.signedUrlExpiresIn ?? null,
    ...
    confidence: attachments.some((file) => file.downloaded === true) && items.length > 0 ? "medium" : "low",
  }
}
