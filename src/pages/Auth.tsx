...
const Auth = () => {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    const {
      error
    } = await updatePassword(updatePasswordForm.password);
    if (!error) {
      navigate("/");
    } else {
      setLoading(false);
  ...
  return <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-primary/5 p-4 border-0 rounded-sm">
        ...
        <CardContent>
          {mode === "login" && <form onSubmit={handleLogin} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="login-email" type="email" placeholder="tuo@email.com" value={loginForm.email} onChange={e => setLoginForm({
              ...loginForm,
          ...
          {mode === "reset" && <form onSubmit={handleResetPassword} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="reset-email" type="email" placeholder="tuo@email.com" value={resetForm.email} onChange={e => setResetForm({
              email: e.target.value
            })} disabled={loading} required />
              </div>
              {error && <p className="text-sm text-destructive">{error}</p>}
          ...
          {mode === "update" && <form onSubmit={handleUpdatePassword} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="new-password" type="password" placeholder="••••••••" value={updatePasswordForm.password} onChange={e => setUpdatePasswordForm({
              ...updatePasswordForm,
              password: e.target.value
            })} disabled={loading} required />
              </div>
              <div className="space-y-2">
                ...
                <Input id="confirm-password" type="password" placeholder="••••••••" value={updatePasswordForm.confirmPassword} onChange={e => setUpdatePasswordForm({
              ...updatePasswordForm,
